...
- **Health import: the merge rules, and the Android half** (PRs #113, #114). Slices 1 and
  2 of `proposals/2026-07-30-health-import.md`.
  SLICE 1 (#113), `src/healthImport.js`, pure so it tests without a base or a phone: gaps
  ...
  basal temperature is the waking one, and nothing dropped silently - invalid, future and
  overflow samples are each counted and returned.
  ONE DELIBERATE DEVIATION from the proposal: provenance is per FIELD (a `sources` map),
  not per row, because a day can hold a flow the user typed AND a BBT from the platform,
  so a row-level marker would be a lie. Compat is identical - `rowApplyDecision` validates
  structure without whitelisting fields, so it replicates unchanged on an older peer.
  SLICE 2 (#114): `plugins/with-android-health-connect.js` declares exactly
  ...
  1. The first import CRASHED the app - `UninitializedPropertyAccessException: lateinit
     ...
     MainActivity edit, but `android/` is generated by prebuild so that edit is wiped every
     build. The config plugin now injects it, and throws if the generated file stops
     matching what it patches.
  2. A FAILED read reported itself as SUCCESS. `readRecords` was wrapped in
