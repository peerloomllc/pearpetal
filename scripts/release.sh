...
#
# Gitignored paths are skipped, not passed to `git add`. In this repo `ios/` is
# regenerated by `expo prebuild` and gitignored, so $XCODE_PROJECT can never be
# committed here even though it IS committed in the sibling app this step was
# ported from. Handing `git add` an ignored path makes it exit 1, and `set -e`
# then aborts the whole release AFTER every build and the final confirm - which
# is exactly what happened on the 1.0.4 run, 2026-07-30.
_bump_existing=()
for _p in "${_bump_paths[@]}"; do
  [ -e "$_p" ] || continue
  if git check-ignore -q -- "$_p"; then
    echo "    Skipping $_p (gitignored - regenerated, not tracked)"
    continue
  fi
  _bump_existing+=("$_p")
done
